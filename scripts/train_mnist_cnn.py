"""
Phase 2 / 3：MNIST + CNN。支援 CLI 超參、run 資料夾、checkpoint、曲線圖、overfitting 判斷。
- 每 epoch 印出 Train/Val Loss、Acc（對齊 sky segmentation 訓練日誌）。
- 偵測中斷：若存在未跑完的 run（同前綴 *_mnist_cnn），則自動續訓。
從專案根目錄執行：python scripts/train_mnist_cnn.py --epochs 10 --save_best
"""
import argparse
import csv
import os
...
def _find_incomplete_cnn_run(out_dir, epochs):
    """找最新且未完成（completed_epochs < epochs）的 *_mnist_cnn run。"""
    runs_dir = out_dir / "runs"
    if not runs_dir.exists():
        return None, 0
    candidates = [d for d in runs_dir.iterdir() if d.is_dir() and d.name.endswith("_mnist_cnn")]
    if not candidates:
        return None, 0
    # 依修改時間取最新
    candidates.sort(key=os.path.getmtime, reverse=True)
    log_path = Path("experiments_cnn.csv")
...
def main():
    p = argparse.ArgumentParser(description="MNIST CNN")
    ...
    if args.run_name:
        ...
        existing_run, completed = _find_incomplete_cnn_run(out_dir, args.epochs)
        if existing_run is not None:
            ...
            run_name = datetime.now().strftime("%Y%m%d_%H%M%S") + "_mnist_cnn"
    ...
    log_path = run_dir / "experiments_cnn.csv"
    ...
    if use_existing:
        last_ckpt = run_dir / "last_mnist_cnn.pt"
        ...
        if ckpt is not None:
            ...
            print("[繼續訓練] 未找到 last_mnist_cnn.pt，從頭開始訓練權重", flush=True)
            print()
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.save_best and val_acc > best_val_acc:
            ...
            save_checkpoint(model, optimizer, epoch, val_acc, str(run_dir / "best_mnist_cnn.pt"))
        save_checkpoint(model, optimizer, epoch, val_acc, str(run_dir / "last_mnist_cnn.pt"))
