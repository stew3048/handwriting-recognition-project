"""
EMNIST ResNet 溫和均衡版：平衡分類準確率與 Embedding 分離度。
- 資料：0, O, 1, I 每類 15,000，其餘 5,000。
- 擴增：僅對 0/O/1/I 做擴增，RandomAffine(degrees=10, translate=(0.1, 0.1))，20% 機率，不做 erasing。
- CE Loss：label_smoothing=0.1，所有類別權重 1.0。
- Center Loss：Epoch 1-5 alpha=0（Warm-up），6-20 線性 0.05→0.12。
- 20 Epoch；Adam + CosineAnnealingLR；batch_size=10。
- 存為 balanced_refined_model.pt。
"""
import argparse
import csv
import sys
...
def main():
    p = argparse.ArgumentParser(description="EMNIST 36 溫和均衡版，存 balanced_refined_model.pt")
    ...
    run_name = datetime.now().strftime("%Y%m%d_%H%M%S") + "_emnist36_balanced_refined"
    ...
    train_loader, val_loader, test_loader = get_emnist36_balanced_refined_loaders(
    ...
    print("=== EMNIST 36 溫和均衡版：僅 0/O/1/I 擴增 20% + CE 權重 1.0 + Center Loss 0.05→0.12 ===", flush=True)
    print(f"Center Loss: 1-5 warmup (alpha=0), 6-20 線性 0.05→0.12", flush=True)
    ...
    try:
        for epoch in range(1, TOTAL_EPOCHS + 1):
            ...
            if val_acc > best_val_acc:
                ...
                save_checkpoint(
                    ...
                    epoch, val_acc, str(run_dir / "balanced_refined_model.pt"),
                )
            save_checkpoint(
                model, criterion_center, optimizer_model, optimizer_center,
                epoch, val_acc, str(run_dir / "last_balanced_refined_model.pt"),
            )
        ...
        print(f"Checkpoints: {run_dir / 'balanced_refined_model.pt'} (best), {run_dir / 'last_balanced_refined_model.pt'}", flush=True)
