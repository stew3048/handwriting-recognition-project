"""
Phase 1 / 3：MNIST + MLP。支援 CLI 超參、checkpoint、augmentation、實驗紀錄。
- 每 epoch 印出 Train/Val Loss、Acc（對齊 sky segmentation 訓練日誌）。
- 偵測中斷：若存在未跑完的 run（同前綴 *_mnist_mlp），則自動續訓。
從專案根目錄執行：python scripts/train_mnist_mlp.py [--epochs 10] [--epochs 1 可快速驗證]
"""
import argparse
import csv
import os
...
def plot_curves(history, save_path, overfitting_msg=""):
    """Plot train/val loss and train/val acc; show overfitting judgment as text on figure."""
...
def judge_overfitting(history):
    """Simple overfitting check: val loss up + train loss down, or large train-val acc gap."""
...
def _find_incomplete_mlp_run(out_dir, epochs):
    """找最新且未完成（completed_epochs < epochs）的 *_mnist_mlp run。"""
    runs_dir = out_dir / "runs"
    if not runs_dir.exists():
        return None, 0
    candidates = [d for d in runs_dir.iterdir() if d.is_dir() and d.name.endswith("_mnist_mlp")]
    if not candidates:
        return None, 0
    candidates.sort(key=os.path.getmtime, reverse=True)
    for run_dir in candidates:
        csv_path = run_dir / "experiments_mlp.csv"
...
def main():
    p = argparse.ArgumentParser(description="MNIST MLP")
    p.add_argument("--epochs", type=int, default=10)
    p.add_argument("--batch_size", type=int, default=64)
    p.add_argument("--lr", type=float, default=1e-3)
    p.add_argument("--dropout", type=float, default=0.0)
    p.add_argument("--augment", action="store_true", help="data augmentation for train")
    p.add_argument("--out_dir", type=str, default=None, help="checkpoint 與 log 目錄，預設 outputs")
    p.add_argument("--save_best", action="store_true", help="僅在 val acc 提升時存 checkpoint")
    p.add_argument("--cpu", action="store_true", help="強制使用 CPU")
    ...
    if args.run_name:
        ...
        existing_run, completed = _find_incomplete_mlp_run(out_dir, args.epochs)
        if existing_run is not None:
            ...
            run_name = datetime.now().strftime("%Y%m%d_%H%M%S") + "_mnist_mlp"
    ...
    log_path = run_dir / "experiments_mlp.csv"
    ...
    model = MLP(input_size=784, hidden_sizes=(256, 128), num_classes=10, dropout=args.dropout).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = Adam(model.parameters(), lr=args.lr)

    if use_existing:
        last_ckpt = run_dir / "last_mnist_mlp.pt"
        ...
        if ckpt is not None:
            ...
            print("[繼續訓練] 未找到 last_mnist_mlp.pt，從頭開始訓練權重", flush=True)
            print()
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.save_best and val_acc > best_val_acc:
            ...
            save_checkpoint(model, optimizer, epoch, val_acc, str(run_dir / "best_mnist_mlp.pt"))
        save_checkpoint(model, optimizer, epoch, val_acc, str(run_dir / "last_mnist_mlp.pt"))
